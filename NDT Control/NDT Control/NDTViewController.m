...
- (void)viewDidLoad
{
    ...
    self.welcomeGrayLogo = [[UIImageView alloc]initWithFrame:CGRectMake(104, 106, 117, 133)];
    
    [self animateWelcomeGrayLogo];
}

-(void)animateWelcomeGrayLogo {
    
    [UIView animateWithDuration:1.0 animations:^{
        self.welcomeGrayLogo.frame = self.colorLogo.frame; 
    } completion:^(BOOL finished) {
        
        [UIView animateWithDuration:1.0 delay:0 options: UIViewAnimationCurveLinear animations:^{
            
            welcomeLabel.alpha = 1.0;
            //welcomeGrayLogo.frame = self.colorLogo.frame;
...
- (IBAction)onButtonPressed:(id)sender {
    ...
    if (!on) {
        buf[1] = 0x01;
        [onButton setBackgroundImage:[UIImage imageNamed:@"lighted_bulb.png"] forState:UIControlStateNormal];
    }
    
    else {
        buf[1] = 0x00;
        [onButton setBackgroundImage:[UIImage imageNamed:@"bulb.png"] forState:UIControlStateNormal];
    }
...
-(void) connectionTimer:(NSTimer *)timer
{
    ...
    if (ble.peripherals.count > 0)
    {
        ...
        [UIView animateWithDuration:1.75 delay:0 options:UIViewAnimationCurveLinear animations:^{
